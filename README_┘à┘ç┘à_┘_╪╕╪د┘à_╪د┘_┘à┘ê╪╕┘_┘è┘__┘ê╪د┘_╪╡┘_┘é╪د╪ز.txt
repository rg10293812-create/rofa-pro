...
def export_data_json():
    data={}
    models=[User,ExternalMarketer,Property,Contact,Offer,CustomerMessage,Sale,Payout,ExternalDeal,DealShare,SystemLog,AppSetting]
    for model in models:
        table=[]
...
def visitor_offers():
    ...
    offer_type=request.args.get('type','all').strip()
    query=Offer.query.filter(Offer.status!='sold')
    if offer_type=='exclusive':
        query=query.filter(Offer.is_exclusive==True)
    if q:
    ...
    exclusive_title=get_setting('exclusive_title','مميزات العروض الحصرية')
    exclusive_features=get_setting('exclusive_features','عروض منتقاة بعناية، تفاصيل أوضح، أولوية في المتابعة، وفرص عقارية مميزة لعملاء روفا جولدن.')
    return render_template('visitor_offers.html', offers=offers_rows, q=q, location=location, max_price=max_price, offer_type=offer_type, exclusive_title=exclusive_title, exclusive_features=exclusive_features)
...
def offers():
    user=get_current_user()
    if request.method=='POST':
        if request.form.get('form_action')=='exclusive_settings':
            if user.role not in ['admin','executive']:
                flash('ليس لديك صلاحية تعديل مميزات العروض الحصرية'); return redirect(url_for('offers'))
            set_setting('exclusive_title', request.form.get('exclusive_title',''))
            set_setting('exclusive_features', request.form.get('exclusive_features',''))
            flash('تم حفظ نص مميزات العروض الحصرية')
        ...
        mid=int(request.form.get('marketer_id') or (user.id if user.role=='marketer' else 0) or 0) or None
        ext_mid=int(request.form.get('ext_marketer_id') or 0) or None
        row=Offer(title=request.form.get('title',''),description=request.form.get('description',''),price=request.form.get('price',''),image_data=uploaded[0] if uploaded else '',images_data=json.dumps(uploaded[1:] if len(uploaded)>1 else [],ensure_ascii=False),status=request.form.get('status','available'),is_exclusive=bool(request.form.get('is_exclusive')),marketer_id=mid, ext_marketer_id=ext_mid)
        db.session.add(row); db.session.commit(); flash('تم إضافة العرض وحفظه'); return redirect(url_for('offers'))
    query=Offer.query
    if user.role=='marketer': query=query.filter_by(marketer_id=user.id)
    return render_template('offers.html', offers=query.order_by(Offer.id.desc()).all(), marketers=ExternalMarketer.query.order_by(ExternalMarketer.name).all(), exclusive_title=get_setting('exclusive_title','مميزات العروض الحصرية'), exclusive_features=get_setting('exclusive_features','عروض منتقاة بعناية، تفاصيل أوضح، أولوية في المتابعة، وفرص عقارية مميزة لعملاء روفا جولدن.'))
...
@app.route('/offers/exclusive/<int:oid>', methods=['POST'])
@login_required
def offer_exclusive(oid):
    if get_current_user().role not in ['admin','executive','marketer']:
        flash('ليس لديك صلاحية تعديل نوع العرض'); return redirect(url_for('offers'))
    row=Offer.query.get_or_404(oid)
    row.is_exclusive=bool(request.form.get('is_exclusive'))
    db.session.commit(); flash('تم تحديث نوع العرض')
    return redirect(url_for('offers'))
